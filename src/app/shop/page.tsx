...
export default function ShopPage() {
  ...
  return (
    <main className="min-h-screen bg-white selection:bg-accent/20 flex flex-col">
      ...
      <section className="relative pt-32 pb-20 md:pt-48 md:pb-32 overflow-hidden bg-[#050505]">
        ...
        <div className="container mx-auto px-6 max-w-6xl text-center relative z-10">
          <div className="reveal-on-scroll active space-y-4 md:space-y-6">
            ...
            <p className="text-white/60 font-medium max-w-lg mx-auto leading-relaxed italic text-xs md:text-lg">
        ...
        <div className="container mx-auto px-4 md:px-6 relative max-w-7xl">
          <div className="relative flex items-center max-w-5xl mx-auto">
            <button 
              ...
              className={cn(
                "absolute -left-8 md:-left-12 top-1/2 -translate-y-1/2 z-50 w-8 h-8 md:w-10 md:h-10 rounded-full bg-white shadow-xl border border-primary/5 flex items-center justify-center text-primary transition-all duration-300",
            ...
            <div 
              ...
              className="flex items-center gap-6 md:gap-10 overflow-x-auto no-scrollbar scroll-smooth py-2 px-4 w-full"
            >
              {categories.map((cat) => (
                <a 
                  key={cat} 
            ...
            <button 
              ...
              className={cn(
                "absolute -right-8 md:-right-12 top-1/2 -translate-y-1/2 z-50 w-8 h-8 md:w-10 md:h-10 rounded-full bg-white shadow-xl border border-primary/5 flex items-center justify-center text-primary transition-all duration-300",
      ...
      <div className="flex-1 pb-20 md:pb-32">
        {isLoading ? (
              ...
              >
                <div className="container mx-auto px-4 md:px-6 max-w-7xl">
                  ...
                  {catProducts.length === 0 ? (
                    <div className="py-16 md:py-24 text-center border-2 border-dashed border-primary/5 rounded-[2rem] md:rounded-[4rem] bg-paper/30">
                       <p className="text-primary/20 font-headline text-xl md:text-2xl italic">"New loops coming soon to this collection."</p>
                    </div>
                  ) : (
                    <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 md:gap-16">
                      {catProducts.map((product) => (
                        <div key={product.id} className="group space-y-4 md:space-y-8">
                          ...
                          <div className="space-y-2 md:space-y-4 text-center">
                            <div className="space-y-0.5 md:space-y-1">
                              <h3 className="font-headline text-lg md:text-3xl text-primary group-hover:text-accent transition-colors truncate px-2">{product.name}</h3>
                              <p className="font-black text-primary/30 text-xs md:text-sm tracking-[0.3em] md:tracking-[0.4em] uppercase italic">₹ {Number(product.price).toLocaleString('en-IN')}</p>
                            </div>
                            
                            <div className="pt-1 md:pt-2">
                              <button 
                                onClick={(e) => handleAddToCart(e, product)}
                                className="inline-flex items-center gap-2 md:gap-4 px-4 md:px-10 py-3 md:py-5 bg-white border border-primary/10 text-primary hover:bg-primary hover:text-white transition-all duration-500 font-black uppercase tracking-[0.3em] md:tracking-[0.5em] text-[8px] md:text-[10px] active:scale-95 group/btn"
                              >
                                <span className="hidden xs:inline">Adopt Selection</span>
                                <span className="xs:hidden">Adopt</span>
