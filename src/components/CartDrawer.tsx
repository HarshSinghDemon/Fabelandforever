  ...
  return (
    <Sheet>
      ...
      <SheetContent className="w-full sm:max-w-md flex flex-col bg-background p-0 border-l border-primary/5 z-[150]">
        <SheetHeader className="p-8 md:p-10 border-b border-primary/10 relative flex flex-row items-center justify-between">
        ...
        <div className="flex-1 overflow-y-auto p-5 md:p-8 space-y-6 no-scrollbar bg-paper">
          {cart.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center p-12 space-y-8">
              <div className="w-20 h-20 md:w-32 md:h-32 bg-primary/5 rounded-full flex items-center justify-center mb-4 animate-float">
                <ShoppingBasket className="w-10 h-10 md:w-16 md:h-16 text-primary/20" />
              </div>
              <p className="text-primary/70 font-black italic text-lg leading-relaxed">"Your basket is currently as light as a cloud."</p>
              <SheetClose asChild>
                <Button variant="outline" className="rounded-full px-10 h-14 font-black uppercase tracking-widest text-[10px] border-primary/20 text-primary">Start Exploring</Button>
              </SheetClose>
            </div>
          ) : (
            cart.map((item) => (
              <div key={item.id} className="relative bg-white p-5 rounded-[2rem] border border-primary/10 flex gap-4 md:gap-6 shadow-sm hover:shadow-md transition-shadow">
                ...
                <div className="flex-1 flex flex-col justify-between py-1 min-w-0 pr-8">
                  <div>
                    ...
                    <p className="text-[9px] md:text-[10px] text-accent font-black uppercase tracking-widest mt-1.5">{item.category}</p>
                  </div>
        ...
        {cart.length > 0 && (
          <div className="p-8 md:p-10 border-t border-primary/10 space-y-6 bg-white shadow-[0_-10px_40px_rgba(0,0,0,0.05)] pb-10">
            ...
            <SheetClose asChild>
              <Button asChild className="w-full h-16 md:h-20 rounded-[2rem] bg-primary hover:bg-primary/90 text-white font-black text-lg uppercase tracking-[0.3em] shadow-2xl shadow-primary/20 transition-all hover:scale-[1.02] active:scale-95">
            ...
            <button 
              onClick={clearCart}
              className="w-full text-[9px] text-primary/40 hover:text-destructive transition-colors uppercase tracking-[0.5em] font-black pb-4"
            >
              Empty Basket
            </button>
          </div>
