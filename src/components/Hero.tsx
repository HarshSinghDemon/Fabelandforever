...
export function Hero() {
  ...
  return (
    <section className="relative h-screen w-full flex items-center justify-center overflow-hidden bg-[#111]">
      {/* Background Image with Darker Overlay for Header Contrast */}
      ...
      <div className="container mx-auto px-6 relative z-10 text-center text-white">
        <div className="max-w-5xl mx-auto space-y-12 animate-fade-in-up">
          <span className="text-[10px] font-bold uppercase tracking-[0.6em] opacity-80 block mb-4">Artisanal Excellence</span>
          
          <h1 className="font-headline text-6xl sm:text-8xl md:text-[10rem] leading-[0.9] mb-6 tracking-tighter">
            A Fable in <br /> <span className="italic">Every Stitch</span>
          </h1>
          
          <p className="text-lg sm:text-2xl max-w-2xl mx-auto leading-relaxed font-medium italic opacity-80">
            "Hand-crocheted treasures that carry the heartbeat of the weaver. Timeless, artisanal, and woven for forever."
          </p>
          
          <div className="pt-12 flex flex-col sm:flex-row items-center justify-center gap-6">
            <Button asChild size="lg" className="bg-white text-primary hover:bg-white/90 px-14 h-16 rounded-full text-[11px] font-bold uppercase tracking-widest transition-all hover:scale-105 active:scale-95 shadow-2xl">
              <Link href="#shop">
                Discover Collection <ArrowRight className="ml-4 w-4 h-4" />
              </Link>
            </Button>
            <Button asChild variant="outline" size="lg" className="border-white/40 text-white hover:bg-white hover:text-primary px-14 h-16 rounded-full text-[11px] font-bold uppercase tracking-widest transition-all">
              <Link href="#custom">Our Process</Link>
            </Button>
          </div>
        </div>
      </div>
      ...
      <div className="absolute bottom-12 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-6 opacity-60">
        <span className="text-[10px] font-bold uppercase tracking-[0.4em] text-white/80 transform rotate-180 [writing-mode:vertical-lr]">Explore</span>
